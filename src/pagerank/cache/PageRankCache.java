...
public final class PageRankCache {
	...
	public void loadFromDB(String tableName) {
		List<ScanSegmentTask> tasks = dynamoWrapper.scanParallel(tableName,
				numSegments);
		...
		for (ScanSegmentTask task : tasks) {
			...
			logger.info("Num of keys from the segment - "
					+ pageRankMap.keySet().size());
		}
	}
