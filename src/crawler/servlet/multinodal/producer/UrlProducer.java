...
public class UrlProducer extends Thread {
	...
	public UrlProducer(MercatorQueue mq, DiskBackedQueue<String> q,
			Integer maxUrls, Map<String, WorkerStatus> workerStatusMap) {
		this.mq = mq;
		this.q = q;
	...
	public Integer getUrlsProcessed(Map<String, WorkerStatus> workerStatusMap) {
		...
		if (sum % (maxUrls / 20) == 0) {
			logger.debug("Checked workers and found a total of " + sum
					+ " urls processed out of " + maxUrls);
		}
		return sum;
	}
	...
	public void run() {
		...
		int timesZero = 0, maxTimesZeroInARow = 10, maxSecsEmpty = 300;
		...
		while (!ThreadPool2.getInstance().isShouldShutdown()) {
			...
			if (mq.getSize() > 0) {
				...
				if (secsEmpty > maxSecsEmpty) {
					System.out
							.println("Queue has been empty for a while. Quitting...");
					shutdown();
					return;
				}
			}
